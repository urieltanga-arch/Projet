{{-- resources/views/admin/promotions/edit-event.blade.php --}}
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
            ...
            <form action="{{ route('admin.events.update', $event) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" 
                              rows="4"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500">{{ old('description', $event->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="type" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                required>
                            <option value="karaoke" {{ old('type', $event->type) === 'karaoke' ? 'selected' : '' }}>Karaoké</option>
                            <option value="football" {{ old('type', $event->type) === 'football' ? 'selected' : '' }}>Football</option>
                        </select>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Participants actuels: {{ $event->current_participants }}</p>
                </div>
