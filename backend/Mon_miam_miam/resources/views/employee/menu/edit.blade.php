    ...
    <div class="py-12 bg-gradient-to-br from-amber-50 to-orange-50 min-h-screen">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-3xl shadow-xl p-8">
                <h1 class="text-3xl font-bold text-black mb-6">Modifier le plat</h1>

                <form action="{{ route('employee.menu.update', $plat) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                class="w-full rounded-xl border-gray-300 focus:border-yellow-500 focus:ring focus:ring-yellow-200"
                            >
                                <option value="plat" {{ old('category', $plat->category) === 'plat' ? 'selected' : '' }}>Plat</option>
                                <option value="boisson" {{ old('category', $plat->category) === 'boisson' ? 'selected' : '' }}>Boisson</option>
                                <option value="dessert" {{ old('category', $plat->category) === 'dessert' ? 'selected' : '' }}>Dessert</option>
                            </select>
                            @error('category')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <input 
                            ...
                            name="image_url" 
                            id="image_url"
                            value="{{ old('image_url', $plat->image_url) }}"
                            class="w-full rounded-xl border-gray-300 focus:border-yellow-500 focus:ring focus:ring-yellow-200"
                        >
                    ...
                    <div class="mb-8">
                        <label class="flex items-center cursor-pointer">
                            <input 
                                ...
                                name="is_available" 
                                value="1"
                                {{ old('is_available', $plat->is_available) ? 'checked' : '' }}
